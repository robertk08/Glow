...
final class CustomProfile {
	...
	var channels: [ProfileChannel] {
		...
		for (index, channel) in channelList.enumerated() {
			channels.append(ProfileChannel(offset: index + 1, role: channel.role, name: channel.name.isEmpty ? channel.role.name : channel.name))
		}
	...
	var profile: FixtureProfile {
		FixtureProfile(id: identifier, model: name, mode: "\(channelList.count) channel", channels: channels, symbol: symbol)
	}
}
