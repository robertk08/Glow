            ...
            if group.members.isEmpty {
                ...
                if control.movesHead {
                    Section("Position") {
                        PositionPad(
                            pan: Binding(
                                get: { control.controls.first?.fraction(.pan) ?? 0.5 },
                                set: { control.setFraction($0, for: .pan) }
                            ),
                            tilt: Binding(
                                get: { control.controls.first?.fraction(.tilt) ?? 0.5 },
                                set: { control.setFraction($0, for: .tilt) }
                            )
                        )
